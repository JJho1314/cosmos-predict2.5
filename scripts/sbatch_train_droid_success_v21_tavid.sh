#!/usr/bin/env bash
# Run Cosmos 2B target-aware post-training on DROID success v2.1 external-camera data.

#SBATCH --job-name=cosmos-v21-tavid
...
#SBATCH --output=/data/user/jhe724/workspace/cosmos-predict2.5/slurm-v21-tavid-%j.out
#SBATCH --error=/data/user/jhe724/workspace/cosmos-predict2.5/slurm-v21-tavid-%j.err
...
export IMAGINAIRE_OUTPUT_ROOT=/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_success_v21_tavid_mask_left_right_split
...
def count_active(dataset_dir):
    ...
    exclude_path = os.path.join(dataset_dir, "exclude_no_tgt_stems.txt")
    excluded = set(open(exclude_path).read().split()) if os.path.exists(exclude_path) else set()
...
train_raw, train_excluded, train_active = count_active(train_dir)
val_raw, val_excluded, val_active = count_active(val_dir)
print("train_dataset:", train_dir)
print("train_videos_raw:", train_raw)
print("train_excluded_no_tgt:", train_excluded)
print("train_videos_active:", train_active)
print("train_masks:", len(os.listdir(os.path.join(train_dir, "masks"))))
print("val_dataset:", val_dir)
print("val_videos_raw:", val_raw)
print("val_excluded_no_tgt:", val_excluded)
print("val_videos_active:", val_active)
print("val_masks:", len(os.listdir(os.path.join(val_dir, "masks"))))
...
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_tavid_mask}
JOB_NAME=${JOB_NAME:-2b_droid_success_v21_tavid_mask_480_lr_split_val1k_49f_s234_actionstart_bs4accum2_14k_val1000}

echo "=== TRAIN DROID success v21 target-aware; experiment=${EXPERIMENT}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; validation=${RUN_VALIDATION}; validation_iter=${VALIDATION_ITER}; max_val_iter=${MAX_VAL_ITER}; job_name=${JOB_NAME} ==="
torchrun --standalone --nproc_per_node=8 -m scripts.train \
  ...
  model.config.net.concat_target_mask=True \
