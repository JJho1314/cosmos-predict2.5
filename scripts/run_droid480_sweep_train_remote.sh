#!/usr/bin/env bash
# Run on ACD1-3 after 480p DROID data has been synced.
...
SUMMARY="outputs/manual_runs/droid480_sweep_train_${STAMP}.summary"
...
for spec in 4:2 2:4 1:8; do
  BS=${spec%:*}
  ACCUM=${spec#*:}
  TEST_JOB_NAME="2b_droid_success_failure_480_bs64_sweep_bs${BS}_accum${ACCUM}_${STAMP}"
  LOG="outputs/manual_runs/droid480_batch_sweep_bs${BS}_${STAMP}.log"
  echo "TEST_BS=$BS GRAD_ACCUM=$ACCUM EFFECTIVE_GLOBAL_BS=64 LOG=$LOG $(date)" | tee -a "$SUMMARY"
  JOB_NAME="$TEST_JOB_NAME" BATCH_SIZE="$BS" GRAD_ACCUM_ITER="$ACCUM" MAX_ITER=10 bash scripts/sbatch_train_droid_success_failure_480.sh > "$LOG" 2>&1
  STATUS=$?
...
TRAIN_LOG="outputs/manual_runs/droid480_train_bs${BEST_BS}_10k_${STAMP}.log"
echo "TRAIN_BS=$BEST_BS GRAD_ACCUM=$BEST_ACCUM EFFECTIVE_GLOBAL_BS=64 LOG=$TRAIN_LOG $(date)" | tee -a "$SUMMARY"
TRAIN_JOB_NAME="2b_droid_success_failure_480_bs64_bs${BEST_BS}_accum${BEST_ACCUM}_10k_${STAMP}"
JOB_NAME="$TRAIN_JOB_NAME" BATCH_SIZE="$BEST_BS" GRAD_ACCUM_ITER="$BEST_ACCUM" MAX_ITER=10000 bash scripts/sbatch_train_droid_success_failure_480.sh > "$TRAIN_LOG" 2>&1
STATUS=$?
