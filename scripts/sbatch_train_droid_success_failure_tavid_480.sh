...
echo "=== TRAIN TAViD-mask 2B on 480x864 droid_success train split + all droid_failure; batch_size=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; max_iter=${MAX_ITER}; job_name=${JOB_NAME} ==="
torchrun --standalone --nproc_per_node=8 -m scripts.train \
  --config=cosmos_predict2/_src/predict2/configs/video2world/config.py \
  -- experiment=predict2_video2world_training_2b_droid_success_failure_tavid_mask \
  job.name="$JOB_NAME" \
